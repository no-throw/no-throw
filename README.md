...
surviving `@nothrow` tags — and the `configs.recommended` preset. Everything
the chain cannot answer floors to throwing with a diagnostic naming your outs,
and every out it names is now a rung you can really reach for. The ES
...
Not built yet: the baseline rung, and `nothrow emit`, which is what writes a
manifest in the first place — so a package you own is colored by hand, from the
project that consumes it, like any other. The design is locked and lives in
