...
if (against !== undefined) {
  const other = (await import(pathToFileURL(against).href)) as { default?: typeof ts };
  before = currentSymbolSet(other.default ?? (other as unknown as typeof ts));
} else {
...
console.log(
  `DOM symbols: ${before.members.length} (TypeScript ${before.typescript}) → ${current.members.length} (TypeScript ${current.typescript})`,
);
