...
import { PACKAGE_SOURCE } from "./baseline/rung.js";
import { bridgeEdit, type BridgeEdit, type BridgeShape } from "./bridge.js";
import type {
  AbsenceReason,
  ConsumptionReason,
  FloorReason,
...
import { findMarks, type MarkProblemKind, type Span } from "./marks.js";
import type { HiddenCallee, TransferSite } from "./transfers.js";

/**
 * This file is the message contract, and it lives in the engine so that every
 * host says the same thing. A host adapter turns these reports into its own
 * diagnostics and suggestions; what it never does is compose a sentence — the
 * outs a floor names are normative, and normative text kept in one place per
 * adapter is normative text that drifts.
 */
...
/** The full catalog a host registers, offer labels included. */
export const escapeMessages: Record<EscapeMessageId, string> = {
  ...diagnostics,
  ...offers,
};

export type EscapeDiagnosticId = keyof typeof diagnostics;
export type EscapeOfferId = keyof typeof offers;
export type EscapeMessageId = EscapeDiagnosticId | EscapeOfferId;
 ...
 */
const bridgeFor: Record<EscapeDiagnosticId, Bridge | undefined> = {
...
/**
 * The whole invariant is one rule, so every escape the analysis reports has to
 * land on a message inside it. A finding kind with no `case` here stops
 * returning a `Report` on every path, which is a compile error.
 */
function reportFor(
  finding: Finding,
  cwd: string,
 ...
 * analysis reports an absolute file name because it has no notion of a project
 * root; a report is addressed to one, and printing paths is not analysis.
 */
...
export interface EscapeOffer {
  readonly messageId: EscapeOfferId;
  readonly edit: BridgeEdit;
}

/**
 * One diagnostic, addressed to whatever host is listening: where the caret
 * goes, what to say, and — where the remedy is a mechanical bridge — the edit
 * to offer. A host maps `anchor` into its own positions and hands `offer.edit`
 * to its own fixer; it adds no judgement of its own.
 */
export interface EscapeReport {
  readonly anchor: Span;
  readonly messageId: EscapeDiagnosticId;
  readonly data: Readonly<Record<string, string>> | undefined;
  readonly offer: EscapeOffer | undefined;
}

/**
 * The offer, where there is one to make. It is never an autofix: wrapping a
 * call in a bridge changes what the program does with an error, and a tool may
 * not make that choice on the reader's behalf — `--fix` would rewrite a whole
 * codebase into one that swallows everything and reports nothing.
 */
function offerFor(
  messageId: EscapeDiagnosticId,
  node: ts.Node,
  sourceFile: ts.SourceFile,
  source: FloorSource,
): EscapeOffer | undefined {
  ...
  const edit = bridgeEdit(node, bridge.shape, sourceFile);
  if (edit === undefined) return undefined;

  return { messageId: bridge.messageId, edit };
}

/** Every escape in the file, as reports a host forwards without judgement. */
export function escapeReports(
  sourceFile: ts.SourceFile,
  program: ts.Program,
  cwd: string,
): readonly EscapeReport[] {
  return analyzeSourceFile(sourceFile, program).map((finding) => {
    const source = sourceOf(finding);
    const report = reportFor(finding, cwd, source);
    return {
      anchor: finding.anchor,
      messageId: report.messageId,
      data: "data" in report ? report.data : undefined,
      // The escape site is what an offered bridge wraps; where the caret goes
      // is `anchor`, and it is narrower wherever the escape is about one
      // member of the expression it was written in.
      offer: offerFor(report.messageId, finding.node, sourceFile, source),
    };
  });
}

/**
 * The mark-hygiene rule, stated rather than enumerated: a list of positions is
 * what produces messages saying a mark binds to nothing at a body sitting
 * right there.
 */
const RULE =
...
export type MarkMessageId = keyof typeof markMessages;

/** An unmapped problem kind is a compile error here, never a dropped report. */
const markMessageIdByKind: Record<MarkProblemKind, MarkMessageId> = {
...
/** A dead or misspelled mark, addressed the same way an escape is. */
export interface MarkReport {
  readonly anchor: Span;
  readonly messageId: MarkMessageId;
  readonly data: Readonly<Record<string, string>>;
}

/** Every mark problem in the file, as reports a host forwards. */
export function markReports(sourceFile: ts.SourceFile): readonly MarkReport[] {
  return findMarks(sourceFile).problems.map((problem) => ({
    anchor: problem.span,
    messageId: markMessageIdByKind[problem.kind],
    data: problem.data,
  }));
}
